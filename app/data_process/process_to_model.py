...
def get_train_test_data() -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    ...
    df = df.drop(["price", 'parking'], axis=1)
    ...
    joblib.dump(df, './staff_data/prepared_dataset.csv')
    ...
    for col in encoders_map:
        ...
        joblib.dump(encoder, f"./staff_data/{col}_encoder.pkl")
